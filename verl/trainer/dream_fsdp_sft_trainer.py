...
from verl.utils.dataset.dream_sft_dataset import dLLMSFTDataset
...
class FSDPdLLMSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        model_dtype = self.config.model.fsdp_config.get("model_dtype", torch.bfloat16)
        ...
        mixed_precision = MixedPrecision(param_dtype=model_dtype, reduce_dtype=torch.float32, buffer_dtype=torch.float32)
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                unsclaed_loss = loss_fct(shift_logits, shift_labels).view(input_ids.shape[0], -1)

                loss = unsclaed_loss / t.unsqueeze(-1)
                loss = loss.reshape(-1) * loss_mask.to(loss.device)
            else:
                ...
                loss_mask = loss_mask_nonflatten
                ...
                input_ids_rmpad_sliced, _, pad_size = ulysses_pad_and_slice_inputs(
                    input_ids_rmpad, None, sp_size=get_ulysses_sequence_parallel_world_size()
                ) # (1, total_nnz // sp) 
                ...
                loss_mask = pad_input(
                    hidden_states=loss_mask_rmpad_rolled.transpose(1, 0), 
                    indices=indices, 
                    batch=batch_size, 
                    seqlen=seqlen
                ).squeeze(-1)  # (batch_size, seqlen)
...
def main(config):
    ...
    val_dataset = create_sft_dataset(config.data.val_files, config.data, tokenizer, eval=True)

    trainer = FSDPdLLMSFTTrainer(config=config, device_mesh=device_mesh, ulysses_device_mesh=ulysses_device_mesh, tokenizer=tokenizer, train_dataset=train_dataset, val_dataset=val_dataset)

    trainer.fit()


def create_sft_dataset(data_paths, data_config, tokenizer, eval=False):
    """Create a dataset."""
    # build dataset
    ...
    # Default to single-turn dataset
        ...
        dataset_cls = dLLMSFTDataset

    # Create datasets based on the selected class
    dataset = dataset_cls(parquet_files=data_paths, tokenizer=tokenizer, config=data_config, eval=eval)
    return dataset
