...
class DreamFlashAttention(DreamAttention):
    """
    Dream attention module using FlashAttention v2 via `transformers.modeling_flash_attention_utils._flash_attention_forward`.
    """
    ...
    def forward(
        ...
        # FlashAttention does not support attention weights, thus keep consistency with DreamSdpaAttention
        if output_attentions:
            logger.warning_once(
                "DreamModel is using DreamFlashAttention, but FlashAttention does not "
                "support `output_attentions=True`. Falling back to the manual attention "
                "implementation. To avoid this warning, load the model with "
        ...
        # FlashAttention require contiguous
        query_states = query_states.contiguous()
        key_states = key_states.contiguous()
        value_states = value_states.contiguous()

        # Note: _flash_attention_forward of transformers expects the shape of mask [bsz, q_len]
        ...
        if is_flash_attn_2_available() and cu_seqlens is not None:
            ...
            # if FlashAttention is not avaiable，callback to PyTorch SDPA (similar to DreamSdpaAttention)
            # match mask's dtype
            attn_mask = None
            if isinstance(attention_mask, torch.Tensor):
                # (bsz, seq_len) 0/1 -> bool/float，broadcast implemented by SDPA
...
class DreamDecoderLayer(nn.Module):
    def __init__(self, config: DreamConfig, layer_idx: int):
        ...
        # self.self_attn = Dream_ATTENTION_CLASSES[config.attn_implementation](config, layer_idx)
        if config._attn_implementation == "flash_attention_2":
            ...
            raise NotImplementedError(f"Unknown attn_implementation: '{config._attn_implementation}'")
